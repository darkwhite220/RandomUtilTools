...
//    val deviceSensors = remember { mutableListOf(sensorManager.getSensorList(Sensor.TYPE_ALL)) }
    Column(
        ...
        if (accelerometer != null) {
            ...
            Text(
                text = "${(x / MAX_VALUE * 100).round(2)}%, " +
                    "${(y / MAX_VALUE * 100).round(2)}%, " +
                    "${(z / MAX_VALUE * 100).round(2)}%"
            )
            val (pitch, roll) = calculatePitchAndRoll(floatArrayOf(x, y, z))
            Text(
                text = "1) Surface Leveled: ${isSurfaceLeveled(pitch, roll)}, " +
                    "${pitch.round(2)}, ${roll.round(2)}"
            )
            
            val (pitch1, roll1) = calculatePitchAndRoll(floatArrayOf(x, z, y))
            Text(
                text = "2) Surface Leveled: ${isSurfaceLeveled(pitch1, roll1)}, " +
                    "${pitch1.round(2)}, ${roll1.round(2)}"
            )
            
            val (pitch2, roll2) = calculatePitchAndRoll(floatArrayOf(z, y, x))
            Text(
                text = "3) Surface Leveled: ${isSurfaceLeveled(pitch2, roll2)}, " +
                    "${pitch2.round(2)}, ${roll2.round(2)}"
            )
            
        } else {
            Text(text = "No Gyroscope Available")
        }
